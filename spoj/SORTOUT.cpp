...
{
	struct TrieNode *pcrawl = root;
	if(pcrawl->isEndofWord) indval+= pcrawl->numcount;
	...
	for(int i = 0; i < 26; i++)
	{
		if(pcrawl->children[i])
		{
			...
			pcrawl->children[i]->indexcount = indval;
			pcrawl->children[i]->minpos = min(pcrawl->children[i]->minpos, indval);
			traverse(pcrawl->children[i]);
			pcrawl->children[i]->indexcount = indval - 1;
			//pcrawl->indexcount = indval;
		}
	}
}
