    ...
    int chain::evaluatescore(int pl[n+1][n+1], int amt[n+1][n+1], int turn,int depth)
    {
    	...
    	for(int i=1;i<=n;i++)
    	{
    		for(int j=1;j<=n;j++)
    		{
    			...
    			{
    				mect+= amt[i][j];
    				if(i-1>0) if(amt[i-1][j] == surct[i-1][j]-1 && pl[i-1][j]==you)	mect-= amt[i][j]*(5-surct[i-1][j]), ct++;
    				if(i+1<=n) if(amt[i+1][j] == surct[i+1][j]-1 && pl[i+1][j]==you) mect-= amt[i][j]*(5-surct[i+1][j]), ct++;
    				if(j-1>0) if(amt[i][j-1] == surct[i][j-1]-1 && pl[i][j-1]==you) mect-= amt[i][j]*(5-surct[i][j-1]), ct++;
    				if(j+1<=n) if(amt[i][j+1] == surct[i][j+1]-1 && pl[i][j+1]==you) mect-= amt[i][j]*(5-surct[i][j+1]), ct++;
    				if(ct == 0) 
    				{	
    					if(surct[i][j] == 2) mect+=3;
    					else if(surct[i][j]==3) mect+=2;
    					if(amt[i][j] == surct[i][j]-1) mect+=2;
    				}
    			}
    			else if(pl[i][j] == you && amt[i][j]>0)
    			{
    				youct+=amt[i][j];
    				if(i-1>0) if(amt[i-1][j] == surct[i-1][j]-1 && pl[i-1][j]==me) youct-= amt[i][j]*(5-surct[i-1][j]),ct++;
    				if(i+1<=n) if(amt[i+1][j] == surct[i+1][j]-1 && pl[i+1][j]==me) youct-= amt[i][j]*(5-surct[i+1][j]),ct++;
    				if(j-1>0) if(amt[i][j-1] == surct[i][j-1]-1 && pl[i][j-1]==me) youct-= amt[i][j]*(5-surct[i][j-1]),ct++;
    				if(j+1<=n) if(amt[i][j+1] == surct[i][j+1]-1 && pl[i][j+1]==me) youct-= amt[i][j]*(5-surct[i][j+1]),ct++;
    				if(ct == 0) 
    				{
    					if(surct[i][j] == 2) youct+=3;
    					else if(surct[i][j]==3) youct+=2;
    					if(amt[i][j] == surct[i][j]-1) youct+=2;
    				}
    			}
    		}
    	}
    	mect+= cont(pl,amt,turn);
