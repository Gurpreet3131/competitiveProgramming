...
int cont(int pl[n+1][n+1],int amt[n+1][n+1], int turn)
{
	int co=0;
	for(int i=1;i<=n;i++)
	{
		...
		for(int j=1;j<=n;j++)
		{
			if(pl[i][j] == turn && amt[i][j] == surct[i][j]-1)
				ct++;
			else
			{
				co+=ct; ct=0;
...
int chain::evaluatescore(int pl[n+1][n+1], int amt[n+1][n+1], int turn,int depth)
{
	...
	for(int i=1;i<=n;i++)
	{
		for(int j=1;j<=n;j++)
		{
			int ct=0, th=5,tw=10,on=20,upper = 10,mul =2;
			if(pl[i][j] == me && amt[i][j]>0)
			{
				if(i-1>0)
				{
					if(amt[i-1][j] == surct[i-1][j]-1 && pl[i-1][j]==you)	mect+= (upper-surct[i][j]*mul), ct++;

				} 
				if(i+1<=n) if(amt[i+1][j] == surct[i+1][j]-1 && pl[i+1][j]==you) mect+= (upper-surct[i][j]*mul), ct++;
				if(j-1>0) if(amt[i][j-1] == surct[i][j-1]-1 && pl[i][j-1]==you) mect+= (upper-surct[i][j]*mul), ct++;
				if(j+1<=n) if(amt[i][j+1] == surct[i][j+1]-1 && pl[i][j+1]==you) mect+= (upper-surct[i][j]*mul), ct++;
				if(amt[i][j] == surct[i][j]-1)
				{
					if(surct[i][j] == 2) ct = on;
					else if(surct[i][j] == 3) ct=tw;
					else ct = th;
				} 
				else ct = 1;
				mect+= amt[i][j]*ct;
			...
			{
				if(i-1>0) if(amt[i-1][j] == surct[i-1][j]-1 && pl[i-1][j]==me) mect-= (upper-surct[i][j]*mul),ct++;
				if(i+1<=n) if(amt[i+1][j] == surct[i+1][j]-1 && pl[i+1][j]==me) mect-= (upper-surct[i][j]*mul),ct++;
				if(j-1>0) if(amt[i][j-1] == surct[i][j-1]-1 && pl[i][j-1]==me) mect-= (upper-surct[i][j]*mul),ct++;
				if(j+1<=n) if(amt[i][j+1] == surct[i][j+1]-1 && pl[i][j+1]==me) mect-= (upper-surct[i][j]*mul),ct++;
	...
	//if(turn == me) mect+= cont(pl,amt,me);
	//else youct+= cont(pl,amt,me);
