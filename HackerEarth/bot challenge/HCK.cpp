...
int reachpos(int ri,int rj)
{
	...
	for(int j=0;j<10;j++)
	{
		if(j==endj || (starti==endi && startj==endj)) continue;			
		if(a[endi][endj]==0)
		{
			dif=fabs(j-endj);
			if(dif==1)
			...
			{
				if(j<endj) checkhorizontalclearpath(j+1,endj-1,endi);
				else checkhorizontalclearpath(endj+1,j-1,endi);
				if(wrong==0)
				{
					yesreach=1;
				}
	...
	for(int i=0;i<10;i++)
	{
		if(i==endi || (starti==endi && startj==endj)) continue;			
		if(a[endi][endj]==0)
		{
			dif=fabs(i-endi);
			if(dif==1)
			...
			{
				if(i<endi) checkverticalclearpath(i+1,endi-1,endj);
				else checkverticalclearpath(endi+1,i-1,endj);
				if(wrong==0)
				{
					yesreach=1;
				}
		...
		for(int ii=0;ii<10;ii++)
		{
			...
			if(a[ii][tj]==0)
			{
				...
				else
				{
					...
					else checkhorizontalclearpath(ti+1,ii-1,tj);
					if(wrong==0)
					{
						yes=reachpos(ii,tj);
					}
