...
void dfsutil(int u, int vis[], int ind)
{
	if(u == ui[ind])
	{
		cycle = 1;
...
void dfsutil2(int u, int vis[], int ind, int maxind, int minm)
{
	if(u == ui[ind])
	{
		cycle = 1;
...
//		cout << "v = " << v << " mval = " << mval << endl;
		if(vis[v] < bfsct && mval >= minm && mval != ind && mval < gmin)
		{
			if(mval > maxind) maxind = mval;
			dfsutil2(v, vis, ind, maxind, minm);
...
void dfs(int ind, int minm)
{
	int maxind = ind;
	dfsutil2(vi[ind], vis, ind, maxind, minm);
}
int main()
{
	int t; scanf("%d", &t);
