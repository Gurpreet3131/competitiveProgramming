...
int main()
{
	...
	for(int i=1;i<=p-1;i++)
	{
		...
		long long int sum1=0,sum2=0,s2,e2,minpart=big,ss=0;
		long long int minsum1=big,minsum2=big;
		int val;
		for(int j=s+1;j<e;j++)
		{
			sum1=0; sum2=0;
			...
			{
				val=j;
				minsum1=sum1; minsum2=sum2;
				minpart=ss;
			}
			else if(ss==minpart)
			{
				if(max(sum1,sum2)<max(minsum1,minsum2))
				{
					val=j; minsum1=sum1; minsum2=sum2;
					minpart=ss;	
				}
			}
		}
	...
	for(int i=1;i<=p-1;i++)
	{
		...
		for(int j=0;j<ind2-1;j++)
		{
			...
			if(end-start==1) continue;
			long long int maxpartsum=0,rpart=0;
			for(int k=0;k<p;k++)
			{
				long long int sum=0;
				...
				if(sum>maxpartsum) maxpartsum=sum,rpart=k;
			}
		...
		for(int j=s+1;j<e;j++)
		{
			for(int k=s;k<e;k++)
			{
				if(k<j) sum1+=colsum[rowpart][k];
				else sum2+=colsum[rowpart][k];
			}
			ss=max(sum1,sum2);
			if(ss<minpart)
			{
			...
			else if(ss==minpart)
			{
				...
				else if(max(sum1,sum2)==max(minsum1,minsum2))
				{
					if(min(sum1,sum2)<min(minsum1,minsum2))
					{
						val=j; minsum1=sum1; minsum2=sum2;
						minpart=ss;
					}
